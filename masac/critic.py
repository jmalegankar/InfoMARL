...
class CustomQFuncCritic(nn.Module):
    ...
    def forward(self, obs:Dict[str, th.Tensor], action:th.Tensor):
        ...
        all_features, _ = self.attn(all_features, all_features, all_features, need_weights=False) # => [n_agents, n_landmarks, hidden_dim]
        all_features = self.layernorm(all_features) + all_features # => [n_agents, n_landmarks, hidden_dim]
