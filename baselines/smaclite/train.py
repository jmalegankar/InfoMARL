...
def run_experiment(args):
    ...
    print(f"Safety: Clip=1.0 (prevents reward spikes), Safe Batching (prevents NaNs)")
    print(f"{'='*60}")
    ...
    if task_name == "2s3z":
        ...
        # Approx max steps for 2s3z is ~120
    elif task_name == "3s5z":
        task = SMACliteTask.THREE_S_FIVE_Z.get_from_yaml()
        # Approx max steps for 3s5z is ~160
    else:
    ...
    # === UPGRADE 1: Convergence Setting ===
    experiment_config.max_n_frames = args.max_frames
    
    # === UPGRADE 2: Visual Tracking ===
    ...
    experiment_config.checkpoint_interval = 200_000 
    experiment_config.evaluation_interval = 50_000
    
    # === LESSON 1: STABILITY (Prevent Crashes) ===
    # SMAC has spikey rewards. High clip (10.0) causes NaNs. Low clip (1.0) is safe.
    ...
    experiment_config.target_update_interval_or_tau = 0.005 if algo_name == "masac" else 200

    # 3. Algorithm Specifics & Batching
    
    # CPU Parallelism Limit (Safe for single machine)
    N_ENVS = 8 
